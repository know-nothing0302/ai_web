...
// 添加收藏
profileRouter.post("/favorites", async (request, response) => {
...
// 取消收藏
profileRouter.delete("/favorites/:articleId", async (request, response) => {
  const userId = request.session.user!.id;
...
// 检查某篇文章是否已收藏
profileRouter.get("/favorites/check/:articleId", async (request, response) => {
  const userId = request.session.user!.id;
  const articleId = request.params.articleId.toString();
  const result = await query<{ id: string; created_at: string }>(
    "SELECT id, created_at FROM user_favorites WHERE user_id = $1 AND article_id = $2 LIMIT 1",
    [userId, articleId]
  );
  if (result.rows.length === 0) {
...
profileRouter.get("/favorites", async (request, response) => {
  ...
  const countResult = await query<CountRow>(
    `SELECT COUNT(*)::text as count FROM user_favorites WHERE user_id = $1`,
    [userId]
  );
  const total = parseInt(countResult.rows[0]?.count ?? "0", 10);

  const itemsResult = await query<FavoriteRow>(
    `SELECT f.id, f.article_id, a.title, a.summary, a.channel_code, a.category, a.published_at, f.created_at
...
// 记录浏览历史
profileRouter.post("/history", async (request, response) => {
...
profileRouter.get("/history", async (request, response) => {
  const parsed = pageSchema.safeParse(request.query);
  if (!parsed.success) {
    response.status(400).json({ message: "参数错误", errors: parsed.error.flatten() });
    return;
  ...
  const countResult = await query<CountRow>(
    `SELECT COUNT(*)::text as count FROM reading_history WHERE user_id = $1`,
    [userId]
  );
  const total = parseInt(countResult.rows[0]?.count ?? "0", 10);

  const itemsResult = await query<HistoryRow>(
    `SELECT h.id, h.article_id, a.title, a.channel_code, a.category, h.viewed_at
