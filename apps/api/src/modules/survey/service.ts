...
const EDIT_QUESTIONS_PROMPT = `你是一个问卷编辑助手。用户会提供一份现有的问卷题目 JSON 数组，以及一条自然语言指令。你需要根据指令修改题目数组。

规则：
- 如果用户要求删除某道题（如"删除第 N 题"），从数组中移除对应题目
- 如果用户要求修改题目内容或选项，修改对应字段
- 如果用户要求新增题目，在指定位置插入
- 如果用户要求调整顺序（如"把第 N 题移到第 M 题后面"），重新排列
- 未被修改的题目请保留其 id 字段不变
- 新添加的题目按顺序生成 id（如 q6、q7...）
- 输出格式：仅输出修改后的题目 JSON 数组，不要输出任何其他文字
- 输出必须是一个合法的 JSON 数组，每个元素是题目对象`;
...
export async function editQuestions(
  ...
  const response = await axios.post(
    `${env.deepseekApiBaseUrl}/v1/chat/completions`,
    {
      model: env.deepseekModel,
      messages: [
        { role: "system", content: EDIT_QUESTIONS_PROMPT },
        {
          role: "user",
          content: JSON.stringify({ questions, instruction }, null, 2),
        },
      ],
      temperature: 0.3,
      max_tokens: 4096,
  ...
  let parsed: SurveyQuestion[];
  try {
    parsed = JSON.parse(jsonText);
  } catch {
    logger.warn("survey.edit.questions.parse_failed", { rawContent: jsonText.substring(0, 500) });
    throw new Error("LLM 返回内容无法解析为 JSON，请重试");
  }
