...
const startContentServer = async (): Promise<{
  server: http.Server;
  baseUrl: string;
}> => {
...
const run = async (): Promise<void> => {
  ...
  axios.post = (async (url, body, config) => {
    if (String(url).includes("/v1/chat/completions")) {
      const prompt = String((body as { messages?: Array<{ content?: string }> })?.messages?.[1]?.content);
            ...
            choices: [
              {
                message: {
                  content:
                    "该文系统梳理了医学人工智能在影像分析、辅助诊疗、教学科研协同和医院治理中的最新应用，重点概括了临床决策支持、数据治理、场景落地与能力建设等关键环节，为高校和医院推进智慧医疗、智能教学与业务协同提供了较为完整的实践参考，也强调了持续优化流程与能力建设的重要性。",
                },
              },
            ],
          },
      ...
      assert.match(prompt, /必须输出一段简短导语/);
      assert.match(prompt, /必须形成 2 到 5 个主章节/);
      assert.match(prompt, /关键事实和数字必须突出/);
            ...
            choices: [
              {
                message: {
                  content: JSON.stringify({
                    summary:
                      "本文综述了人工智能在疾病诊断中的变革性应用，重点涵盖癌症、阿尔茨海默病和糖尿病。通过2022至2024年间文献的系统计量分析（GraphRAG方法），展示了AI在医学影像和分子数据分析中的突破。癌症早期检测覆盖19种类型；阿尔茨海默病风险检测准确率高达90%；糖尿病预测在症状出现前已显成效。尽管面临标准化和数据质量挑战，AI正推动诊断向更精准、高效和可及的方向转变。",
                    content:
          ...
          choices: [
            {
              message: {
                content: JSON.stringify({
                  summary:
                    "该文系统梳理了医学人工智能在影像分析、辅助诊疗、教学科研协同和医院治理中的最新应用，重点概括了临床决策支持、数据治理、场景落地与能力建设等关键环节，为高校和医院推进智慧医疗、智能教学与业务协同提供了较为完整的实践参考，也提示了持续建设基础设施与规范流程的重要性。",
                  content:
                    "医学人工智能正在从**单点辅助**走向教学、科研和临床协同应用。\n## 应用进展\n一是**医学影像分析**能力持续成熟。二是**智能辅助诊疗**覆盖更多场景。三是**临床决策支持**与数据治理协同推进。\n## 实践价值\n对于推进**智慧医疗**与医学教育协同发展，这些实践经验具有较强参考价值。",
                  channelCode: "medical-frontier",
                  author: "徐医融媒中心",
                }),
              },
  ...
  try {
    ...
    assert.match(String(extract.data.content), /医学人工智能正在从.*走向/);
    assert.match(String(extract.data.content), /^##\s+/m);
    assert.match(String(extract.data.content), /^医学人工智能正在从.*\n\n## 应用进展/m);
    assert.match(String(extract.data.content), /## 应用进展\n\n- \*\*医学影像分析\*\*能力持续成熟。/);
    assert.match(String(extract.data.content), /- \*\*智能辅助诊疗\*\*覆盖更多场景。/);
    assert.match(String(extract.data.content), /- \*\*临床决策支持\*\*与数据治理协同推进。/);
    ...
    assert.match(String(extract.data.sourceContent), /这是用于提取测试的正文第一段/);
    assert.doesNotMatch(String(extract.data.sourceContent), /^##\s+/m);
    assert.notEqual(
    ...
    assert.equal(created.data.author, "徐医融媒中心");
    assert.equal(created.data.sourceContent, extract.data.sourceContent);

    const pmcPage = await axios.post(
      `${baseUrl}/api/articles/extract-from-url`,
      { url: `${contentServer.baseUrl}/pmc` },
      { validateStatus: () => true }
    );
    ...
    assert.equal(pmcPage.data.publishedAt, "2025-04-11T00:00:00.000Z");
    assert.ok(pmcPage.data.summary.length >= 120);
    assert.ok(pmcPage.data.summary.length <= 250);
    assert.match(String(pmcPage.data.content), /^##\s+/m);
    assert.match(String(pmcPage.data.content), /\*\*.+\*\*/);
    assert.match(String(pmcPage.data.content), /人工智能/);

    const shortPage = await axios.post(
      `${baseUrl}/api/articles/extract-from-url`,
      { url: `${contentServer.baseUrl}/short` },
      { validateStatus: () => true }
    );
    assertStatus(shortPage.status, 422, "extract short article", shortPage.data);
    assert.match(String(shortPage.data.message), /正文|内容/);
    process.stdout.write("article extract flow test passed\n");
  } finally {
    axios.post = originalPost;
    await new Promise<void>((resolve, reject) => {
      apiServer.close((error?: Error) => {
        if (error) {
          reject(error);
          return;
        }
    ...
    await new Promise<void>((resolve, reject) => {
      contentServer.server.close((error?: Error) => {
        if (error) {
          reject(error);
          return;
        }
