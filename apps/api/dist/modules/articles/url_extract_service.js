...
exports.extractArticleFromUrl = void 0;
...
const aiExtractSchema = zod_1.z.object({
    content: zod_1.z.string().trim().min(80).max(5000),
    channelCode: zod_1.z.string().trim().min(1).max(64),
    author: zod_1.z.union([zod_1.z.string().trim().max(200), zod_1.z.literal("")]).optional(),
});
const aiSummarySchema = zod_1.z.object({
    summary: zod_1.z.string().trim().min(1).max(400),
});
...
const normalizeSummary = (content) => {
...
const normalizeMarkdownEmphasis = (line) => line.replace(/\*\*\s+([^*]+?)\s+\*\*/g, "**$1**");
const expandEnumeratedParagraph = (line) => {
    const markerPattern = /(一是|二是|三是|四是|五是|六是|七是|八是|九是|十是|首先|其次|再次|最后)/g;
...
const pickPublishedAt = ($) => parsePublishedAt($('meta[property="article:published_time"]').attr("content") ||
...
const parseAiJson = (content) => {
    ...
    const candidate = startedAt >= 0 && endedAt > startedAt
...
const parsePublishedAt = (value) => {
    ...
    const dateOnlyMatch = normalized.match(/^(\d{4})[-/\s]+([A-Za-z]{3,9}|\d{1,2})[-/\s,]+(\d{1,2})$/);
    if (dateOnlyMatch) {
        const monthLookup = {
            january: 0,
            jan: 0,
        ...
        const month = monthLookup[monthValue] ?? (Number(monthValue) >= 1 ? Number(monthValue) - 1 : NaN);
...
const buildPrompt = (title, content, author, channels) => `
...
`;
const buildSummaryPrompt = (content) => `请将以下内容提炼为120-180字中文摘要，要求简洁、准确、可发布，保留核心结论与应用价值，不要分点，不要添加标题：\n${content}`;
const requestAiExtraction = async (title, content, author) => {
    ...
    try {
        ...
        logger_1.logger.info("article.extract.ai.extract.success", {
        ...
        if (!parsed.success) {
            logger_1.logger.error("article.extract.ai.extract.invalid_payload", {
                issues: parsed.error.issues.map((issue) => ({
                    path: issue.path.join("."),
                    message: issue.message,
                })),
                payloadPreview: normalizeAiContent(result.data?.choices?.[0]?.message?.content).slice(0, 500),
            });
        ...
        logger_1.logger.error("article.extract.ai.extract.failed", {
            error,
        });
...
const requestAiSummary = async (content) => {
    const startedAt = Date.now();
    try {
        const result = await axios_1.default.post(`${env_1.env.deepseekApiBaseUrl}/v1/chat/completions`, {
            model: env_1.env.deepseekModel,
        ...
        if (!parsed.success) {
            logger_1.logger.error("article.extract.ai.summary.invalid_payload", {
                issues: parsed.error.issues.map((issue) => ({
                    path: issue.path.join("."),
                    message: issue.message,
                })),
        ...
        logger_1.logger.info("article.extract.ai.summary.success", {
            durationMs: Date.now() - startedAt,
        ...
        logger_1.logger.error("article.extract.ai.summary.failed", {
            error,
        });
        if (error.status) {
            throw error;
...
const extractArticleFromUrl = async (input) => {
    logger_1.logger.info("article.extract.start", {
        url: input.url,
    ...
    let html = "";
    try {
        const fetchResult = await axios_1.default.get(input.url, {
            responseType: "text",
            timeout: 10000,
        });
        html = fetchResult.data;
        logger_1.logger.info("article.extract.fetch.success", {
            url: input.url,
        ...
        logger_1.logger.error("article.extract.fetch.failed", {
            url: input.url,
            error,
        });
        if (axios_1.default.isAxiosError(error) && error.code === "ECONNABORTED") {
    ...
    logger_1.logger.info("article.extract.clean.success", {
        url: input.url,
    ...
    logger_1.logger.info("article.extract.author.detected", {
        url: input.url,
    ...
    const aiResult = await requestAiExtraction(extracted.title, extracted.content, extracted.author);
...
exports.extractArticleFromUrl = extractArticleFromUrl;
